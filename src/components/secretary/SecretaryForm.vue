    ...
    <v-form @submit.prevent="submitForm" ref="form">
        ...
        <v-window v-model="activeTab" class="mt-6">
            <v-window-item class="pa-5" value="personal">
                <v-row>
                    <v-col cols="12" sm="6">
                        <v-text-field v-model="secretaryData.firstName" :rules="[validationRules.required]"
                            label="First Name*" placeholder="Enter first name" prepend-inner-icon="mdi-account"
                            required></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6">
                        <v-text-field v-model="secretaryData.lastName" :rules="[validationRules.required]"
                            label="Last Name*" placeholder="Enter last name" prepend-inner-icon="mdi-account"
                            required></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6">
                        <v-select v-model="secretaryData.gender" :items="genderOptions" item-title="title"
            ...
            <v-window-item class="pa-5" value="contact">
                <v-row>
                    <v-col cols="12" sm="6">
                        <v-text-field v-model="secretaryData.phone"
                    ...
                    <v-col cols="12" sm="6">
                        <v-text-field v-model="secretaryData.email"
...
const secretaryData = reactive({ ...props.secretary });
const avatarFile = ref(null);
const avatarPreview = ref(null);

// Initialize avatar preview with existing photo URL when in update mode
watch(() => props.secretary, (newValue) => {
    Object.assign(secretaryData, newValue);
    // Set the avatar preview if the secretary has a photoUrl
...
const submitForm = async () => {
    ...
    if (validation?.valid) {
        ...
        // Add all secretary data fields to the FormData
        for (const [key, value] of Object.entries(secretaryData)) {
